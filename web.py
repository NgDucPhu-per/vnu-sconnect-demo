...
def init_db():
    ...
    # Dữ liệu mẫu 
    sample_users = [
        ('vnusconnect@vnu.edu.vn', 'Giảng viên', 0, 0, 'Trường Đại học Công nghệ', 'admin'),
        ('25022976@vnu.edu.vn', 'Nguyễn Đức Phú', 298, 0, 'Trí tuệ nhân tạo (UET)', 'student'),
        ('25022987@vnu.edu.vn', 'Đào Nhật Quang', 255, 0, 'Trí tuệ nhân tạo (UET)', 'student'),
        ('25022800@vnu.edu.vn', 'Lê Quỳnh Chi', 216, 0, 'Trí tuệ nhân tạo (UET)', 'student'),
        ('25041102@vnu.edu.vn', 'Sinh viên 1', 30, 0, 'Kinh tế quốc tế (UEB)', 'student'),
        ('25060543@vnu.edu.vn', 'Sinh viên 2', 85, 0, 'Báo chí (USSH)', 'student'),
        ('25010099@vnu.edu.vn', 'Sinh viên 3', 10, 0, 'Luật học (VUL)', 'student'),
    ...
    with tab_duyet:
        ...
        if not df_pending.empty:
            for _, row in df_pending.iterrows():
                with st.expander(f"🔴 [CHỜ DUYỆT] {row['title']} - Đăng bởi: {row['uploader']}"):
                    ...
                    with col1:
                        if st.button("✅ Phê duyệt", key=f"app_{row['id']}"):
                            # Đổi trạng thái thành approved và cộng 10 điểm cho người đăng
...
# ---------------------------------------------------------
    ...
    tab_lib, tab_mentor, tab_upload, tab_ranking = st.tabs([
        "📚 Thư viện", 
        "🤝 Mentor", 
        "📤 Đóng góp Tài liệu", 
        "🏆 Bảng vàng"
    ...
    with tab_lib:
        st.subheader("📚 Kho tài liệu học thuật (Đã kiểm duyệt)")
        st.markdown("---")
                ...
                with st.expander(f"📄 {row['title']} | Mã HP: {row['subject_code']}"):
                    ...
                    with col_vote:
                        ...
                        if st.button("✨ Tặng 1 Sao", key=f"star_{row['id']}"):
                            # Gọi hàm cộng vote và thưởng điểm cho người đăng
                            cursor = conn.cursor()
                            cursor.execute("UPDATE documents SET upvotes = upvotes + 1 WHERE id = ?", (row['id'],))
                            conn.commit()
                            
                            # Thưởng thêm cho người đăng tài liệu khi có người vote (Ví dụ +2 điểm)
    ...
    with tab_upload:
        st.subheader("Số hóa & Chia sẻ Tri thức")
        st.info("Tài liệu của bạn sẽ được gửi tới Ban Cố Vấn (Admin) để kiểm duyệt. Nếu hợp lệ, bạn sẽ nhận **+10 VNU-Credit**.")
        
        doc_title = st.text_input("Tên tài liệu:")
        subject_code = st.text_input("Mã học phần:")
        category = st.selectbox("Thuộc đơn vị quản lý môn học:", ["VNU-UET", "VNU-USSH", "VNU-HUS", "VNU-UEB", "VNU-ULIS", "VNU-UED", "VNU-VJU", "VNU-UMP", "VNU-UL"])
        ...
        if st.button("Gửi chờ duyệt"):
            if doc_title and subject_code and drive_link:
                ...
                cursor = conn.cursor()
                # LƯU Ý: Status mặc định là 'pending' (chờ duyệt)
                ...
                conn.commit()
                st.success("Tải lên thành công! Tài liệu đang chờ Admin phê duyệt.")
                # KHÔNG cộng điểm ở đây nữa
            else:
                st.warning("Vui lòng điền đầy đủ các trường thông tin.")
                
    with tab_mentor:
        st.subheader("Hệ thống Ghép đôi Đồng đẳng")
        st.markdown("Tìm kiếm người hướng dẫn hoặc đồng đội ngay tại khu vực của bạn.")
        ...
        if action == "Tìm Mentor / Đồng đội":
            ...
            m_subjects = st.text_input("Môn học bạn tự tin nhất (VD: Giải tích 1, C++, Tiếng Anh B2):")
            ...
            if st.button("Đăng ký ngay"):
                if m_subjects and m_contact:
                    cursor = conn.cursor()
                    # Sử dụng st.session_state.current_user để tự động lấy định danh người đang đăng nhập
                    ...
                    conn.commit()
                    st.success("Đăng ký thành công! Hồ sơ Mentor của bạn đã được hiển thị trên hệ thống.")
                else:
                    st.warning("Vui lòng điền đủ các thông tin chuyên môn và liên hệ.")
